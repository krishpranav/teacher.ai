...
export async function handler(event) {
  ...
  if (!imageUrl) {
    return {
      ...
      body: JSON.stringify({
        success: false,
        message: 'MSG: Image URL parameter is required',
      }),
    };
  }
  ...
  try {
    const base64Image = await new Promise((resolve, reject) => {
      httpModule.get(imageUrl, (res) => {
        if (res.statusCode !== 200) {
          reject(new Error(`ERROR: Request failed with status code ${res.statusCode}`));
          return;
        }
    ...
    return {
      ...
      body: JSON.stringify({
        success: false,
        message: `ERROR: Failed to fetch image: ${error.message}`,
      }),
    };
  }
}